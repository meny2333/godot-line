...
static func parse_text(text: String) -> Dictionary:
	...
	var animator_names: Array[String] = []
	var controller_names: Array[String] = []
	var animator_count := int(raw.get("animator_count", "0"))
	for i in range(animator_count):
		animator_names.append(raw.get("animator_%d_name" % i, ""))
		controller_names.append(raw.get("controller_%d_name" % i, ""))

	var motion_names: Array[String] = []
	var motion_count := int(raw.get("motion_count", "0"))
	for i in range(motion_count):
		motion_names.append(raw.get("motion_%d_name" % i, ""))

	result["animator_names"] = animator_names
	result["controller_names"] = controller_names
	result["motion_names"] = motion_names
	result["animator_count"] = animator_count

...
#### 关卡标题本地化优化
**更新**：关卡标题已从'测试场景'简化为'测试'，反映本地化精简需求

DefaultScene 中的关卡配置
- **levelTitleKey**：保持为 "Default"，用于国际化标识
- **levelTitle**：从 "测试场景" 简化为 "测试"
- **authors**：作者信息保持不变

```mermaid
flowchart TD
Start(["关卡标题配置"]) --> Config["Default.tres 配置"]
Config --> Key["levelTitleKey<br/>\"Default\""]
Config --> Title["levelTitle<br/>\"测试\""]
Config --> Authors["authors<br/>作者信息"]
Title --> Localize["本地化显示"]
Localize --> Simplify["简化显示文本"]
```
...
- **本地化问题**
  - **关卡标题显示异常**：检查 Default.tres 中的 `levelTitle` 字段是否正确设置为 "测试"
...
- **新增**：关卡标题本地化精简，从'测试场景'简化为'测试'，优化用户体验
...
- **本地化配置**
  - **关卡标题**：Default.tres 中的 `levelTitle` 设置为 "测试"

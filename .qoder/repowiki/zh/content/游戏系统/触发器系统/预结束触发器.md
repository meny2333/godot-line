...
## 更新摘要
**所做更改**
- 更新了预结束触发器的结局序列实现，显式设置_firstDirection_和_secondDirection_为相同值以锁定玩家方向
- 直接调用_body.turn()而非使用call_deferred延迟执行
- 设置_body.allowTurn = false以防止结局期间的方向变化
- 优化了相机跟随器的集成，移除了对GameManager的依赖，直接通过CameraFollower.instance访问
- 改进了角度计算和旋转逻辑，确保预结束动画的稳定性
...
**更新** 该系统现已完全实现，包含了简化的PreEnding.gd脚本，移除了对GameManager的依赖，直接通过CameraFollower.instance获取相机跟随器实例，实现了更加直接和高效的相机控制系统。最新的优化包括显式锁定玩家方向、直接调用turn()方法和改进的角度计算逻辑。
...
**图表来源**
- [PreEnding.gd:1-32](file://#Template/[Scripts]/Trigger/PreEnding.gd#L1-L32)
...
**更新** 最新的实现优化了结局序列的执行流程，通过显式设置_firstDirection_和_secondDirection_为相同值来锁定玩家方向，直接调用_body.turn()方法启动移动，而不是使用call_deferred延迟执行。同时设置了_body.allowTurn = false来防止结局期间的方向变化。
...
- [PreEnding.gd:6-32](file://#Template/[Scripts]/Trigger/PreEnding.gd#L6-L32)
- [State.gd:55-100](file://#Template/[Scripts]/State.gd#L55-L100)
...
Trigger->>Player : 设置 firstDirection = face_dir
Trigger->>Player : 设置 secondDirection = face_dir
Trigger->>Player : 直接调用 turn() 启动移动
Trigger->>Player : 设置 allowTurn = false
Trigger->>Player : 设置 is_end = true
...
**图表来源**
- [PreEnding.gd:10-32](file://#Template/[Scripts]/Trigger/PreEnding.gd#L10-L32)
- [State.gd:55-100](file://#Template/[Scripts]/State.gd#L55-L100)
- [CameraFollower.gd:134-150](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L134-L150)
- [Player.gd:193-217](file://#Template/[Scripts]/Level/Player.gd#L193-L217)

该架构的关键优势在于其简化的依赖关系：预结束触发器直接依赖CameraFollower.instance，避免了通过GameManager中间层的间接访问。这种设计使得系统更加直接和高效，减少了不必要的抽象层次。

**更新** 新的执行流程通过显式锁定玩家方向确保了预结束动画的稳定性，直接调用turn()方法启动移动避免了延迟执行可能引起的时序问题，allowTurn标志位防止了玩家在结局期间的意外操作。
...
RoundAngle --> SetDirections["设置 firstDirection = secondDirection = face_dir"]
SetDirections --> DirectTurn["直接调用 turn() 启动移动"]
DirectTurn --> DisableTurn["设置 allowTurn = false"]
DisableTurn --> SetEndState["设置 is_end = true"]
SetEndState --> EmitSignal["触发完成信号"]
EmitSignal --> End
```
...
**更新** 新的执行流程通过显式锁定玩家方向确保了预结束动画的稳定性：
1. 首先计算玩家应该面对的方向
2. 将firstDirection和secondDirection都设置为相同值，确保玩家无法改变方向
3. 直接调用turn()方法启动移动，避免了延迟执行的复杂性
4. 设置allowTurn = false防止玩家在结局期间进行任何操作
5. 这种顺序确保了预结束动画和移动逻辑的正确执行
...
AnimationPlaying --> DirectMovement : "直接调用 turn() 移动"
DirectMovement --> EndSequence : "设置 allowTurn = false"
EndSequence --> NormalGame : "动画完成"
...
[*] --> DirectCameraAccess
DirectCameraAccess --> LockPlayerDirection
LockPlayerDirection --> PlayAnimation
PlayAnimation --> [*]
}
```
...
**更新** 新的执行机制移除了call_deferred的使用，直接调用turn()方法确保了预结束动画和移动逻辑的正确时序。
...
**更新** 新的执行机制进一步优化了性能：
- 直接调用turn()方法避免了call_deferred的调度开销
- 显式锁定玩家方向减少了状态切换的复杂性
...
- 验证显式锁定玩家方向的逻辑是否正确执行
- 检查direct turn()调用是否正确启动移动
- 确认allowTurn标志位是否正确设置为false

**玩家方向锁定问题**：
- 确认firstDirection和secondDirection都被设置为相同值
- 检查玩家的current_direction属性是否正确返回face_dir
- 验证_player.allowTurn标志位是否阻止了输入处理
- 确认turn()方法在预结束期间不会被意外调用

**章节来源**
- [BaseTrigger.gd:24-35](file://#Template/[Scripts]/Trigger/BaseTrigger.gd#L24-L35)
- [PreEnding.gd:10-32](file://#Template/[Scripts]/Trigger/PreEnding.gd#L10-L32)
- [State.gd:90-100](file://#Template/[Scripts]/State.gd#L90-L100)
- [CameraFollower.gd:134-150](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L134-L150)
- [Player.gd:124-128](file://#Template/[Scripts]/Level/Player.gd#L124-L128)
...
6. **执行流程优化**：通过显式锁定玩家方向和直接调用turn()确保了操作的正确时序

**更新** 预结束触发器作为游戏体验的重要组成部分，现在包含了简化的相机跟随器集成、优化的旋转逻辑和改进的执行流程，移除了对GameManager的依赖，为玩家提供了从正常游戏到结束阶段的自然过渡，是整个Godot-Line项目中不可或缺的关键组件。

该系统现已完全实现，包含了所有预期的功能特性，为开发者提供了可靠的预结束触发器解决方案。新的架构设计不仅提高了系统的性能，还简化了代码结构，为未来的维护和扩展奠定了良好的基础。

**更新** 最新的优化进一步提升了系统的稳定性和性能，通过显式锁定玩家方向确保了预结束动画的稳定性，通过直接调用turn()方法避免了延迟执行的复杂性，通过allowTurn标志位防止了玩家在结局期间的意外操作，为玩家提供了更加流畅和一致的游戏体验。
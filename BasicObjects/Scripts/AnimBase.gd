...
func _ready() -> void:
	_initialized = true
	var is_restoring_checkpoint := not Engine.is_editor_hint() and bool(State.get("is_restoring_checkpoint"))
	
	# 编辑器模式：自动记录当前值作为起始值
	if Engine.is_editor_hint() and is_setting_start:
		start_value = _get_value()
	elif not is_restoring_checkpoint:
		# 运行时（非复活恢复）：应用起始值
		_set_value(start_value)

	# 复活恢复时也要保证触发器连接可用，但不能覆盖恢复后的位置
	if trigger and trigger.has_signal("hit_the_line"):
		var callback := Callable(self, "play_")
		if not trigger.is_connected("hit_the_line", callback):
			trigger.connect("hit_the_line", callback)

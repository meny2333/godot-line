...
- **重大重构**：更新检查点处理接口，从旧的命名约定更新为新的描述性名称
...
- load_to_camera_follower()：从相机检查点恢复相机参数
...
### 检查点处理系统重构详解
**重大更新**：State.gd 的检查点处理系统经历了重大重构，主要体现在以下方面：

#### 接口命名标准化
- **旧接口**：save_to_main_line()、load_to_main_line()
- **新接口**：save_checkpoint_to_main_line()、load_checkpoint_to_main_line()
- **改进**：使用更具描述性的命名，明确接口的功能和作用域

#### 统一检查点管理
- **单一入口**：所有检查点操作通过 save_checkpoint() 统一处理
- **数据完整性**：确保相机参数、主线路线、音乐播放位置等数据的一致性
- **字典序列化**：使用高效的字典序列化方法替代多函数系统

#### 恢复机制优化
- **分层恢复**：load_checkpoint_to_main_line() 和 load_runtime_to_main_line() 实现分层状态恢复
- **相机参数恢复**：load_to_camera_follower() 专门处理相机参数的恢复
- **运行时数据管理**：main_line_data 字典专门管理仅场景存活期间有效的数据

```mermaid
flowchart TD
A["save_checkpoint()"] --> B["保存 main_line_transform"]
A --> C["保存 camera_checkpoint"]
A --> D["保存 anim_time"]
A --> E["保存 music_checkpoint_time"]
B --> F["load_checkpoint_to_main_line()"]
C --> G["load_to_camera_follower()"]
D --> H["load_runtime_to_main_line()"]
E --> I["音乐播放恢复"]
```

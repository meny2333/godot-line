...
const Parser := preload("res://addons/mpm_importer/cameratrigger_mpm_parser.gd")
const Importer := preload("res://addons/mpm_importer/cameratrigger_importer.gd")

@export var default_set_camera: NodePath
@export var transform_fix: bool = true

@export_tool_button("Import CameraTrigger Folder") var import_action = func():
...
func _import_from_folder(dir: String) -> void:
	...
	for file_name in files:
		...
		var report: Dictionary = Importer.apply_entry(root, entry, default_set_camera, transform_fix)
	...
	print("CameraTrigger Import Summary")

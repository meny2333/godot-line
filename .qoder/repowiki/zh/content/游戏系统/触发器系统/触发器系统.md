...
- 内置触发器的实现细节：Crown（皇冠）、Diamond（钻石）、Jump（跳跃）、ChangeSpeedTrigger（速度变化）、ChangeTurn（转向变化）、Trigger（通用触发器）、CrownSet（多皇冠状态管理）等
- 触发器的激活条件、执行时机与参数配置
- 与游戏状态管理 State 的交互机制
...
- 状态管理：State.gd
- 辅助工具：GameManager.gd
- 场景资源：Crown.tscn、Diamond.tscn、CrownSet.tscn、trigger.tscn
...
- [Crown.gd:25-40](file://#Template/[Scripts]/Trigger/Crown.gd#L25-L40)
...
## 结论
触发器系统通过 BaseTrigger 提供统一的触发框架，结合多种派生触发器满足不同玩法需求。**新增的 CrownSet 触发器组件完善了多皇冠状态管理系统，通过与 State.crowns 数组的协作，实现了精确的多组皇冠状态跟踪与视觉反馈。** 收集型触发器与 State 的紧密协作实现了清晰的状态管理，而通用触发器则提供了灵活的信号广播能力。遵循本文的扩展指南与最佳实践，可快速开发新的触发器类型并保持系统的可维护性与性能。

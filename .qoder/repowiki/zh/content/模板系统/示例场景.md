# 示例场景

<cite>
**本文引用的文件**
- [README.md](file://README.md)
- [CONTRIBUTING.md](file://CONTRIBUTING.md)
- [project.godot](file://project.godot)
- [plugin.gd](file://addons/gdUnit4/plugin.gd)
- [plugin.cfg](file://addons/gdUnit4/plugin.cfg)
- [GameManager.gd](file://#Template/[Scripts]/GameManager.gd)
- [MainLine.gd](file://#Template/[Scripts]/Level/MainLine.gd)
- [State.gd](file://#Template/[Scripts]/State.gd)
- [RoadMaker.gd](file://#Template/[Scripts]/Level/RoadMaker.gd)
- [gameui.gd](file://#Template/[Scripts]/Level/gameui.gd)
- [Crown.gd](file://#Template/[Scripts]/Trigger/Crown.gd)
- [Diamond.gd](file://#Template/[Scripts]/Trigger/Diamond.gd)
- [Trigger.gd](file://#Template/[Scripts]/Trigger/Trigger.gd)
- [CameraFollower.gd](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd)
- [CameraTrigger.gd](file://#Template/[Scripts]/CameraScripts/CameraTrigger.gd)
- [CamShaker.gd](file://#Template/[Scripts]/CameraScripts/CamShaker.gd)
- [MainLine_test.gd](file://Tests/MainLine_test.gd)
- [Crown_test.gd](file://Tests/Crown_test.gd)
- [Default.tscn](file://#Template/[Scenes]/DefaultScene/Default.tscn)
- [Default.tres](file://#Template/[Scenes]/DefaultScene/Default.tres)
- [Sample.tscn](file://#Template/[Scenes]/Sample/Sample.tscn)
- [Sample.tres](file://#Template/[Scenes]/Sample/Sample.tres)
- [Scene.tscn](file://#Template/[Scenes]/Scene.tscn)
- [MainLine.tscn](file://#Template/MainLine.tscn)
- [CrownCheckPoint.tscn](file://#Template/CrownCheckPoint.tscn)
- [level_patch_meta.tres](file://#Template/[Resources]/level_patch_meta.tres)
</cite>

## 更新摘要
**所做变更**
- 更新场景组织结构章节以反映从独立 Sample.tscn 到 DefaultScene 结构的重大重组
- 更新相机系统章节以反映从 ease_type 到 tween_transition 和 tween_ease 的新属性系统的完整迁移
- 更新场景配置部分以反映新的 DefaultScene/Default.tres 和 DefaultScene/Default.tscn 结构
- 更新相机系统配置指南以反映新的 Tween 属性系统
- 更新故障排除指南中的相机系统问题部分
- **更新**：修复BaseFloor3对象的位置坐标Y坐标精度问题，从2.4076805调整为2.3337727
- **更新**：关卡标题本地化优化，DefaultScene和Sample场景的levelTitle统一为"测试场景"，提供更描述性的中文标题

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [场景组织结构](#场景组织结构)
7. [相机系统详解](#相机系统详解)
8. [依赖关系分析](#依赖关系分析)
9. [性能考量](#性能考量)
10. [故障排除指南](#故障排除指南)
11. [结论](#结论)
12. [附录](#附录)

## 简介
本项目是一个基于 Godot Engine 4.6 的 Dancing Line 游戏模板框架，目标是提供一套开箱即用、模块化的线条游戏实现，支持快速关卡迁移与跨平台发布。项目内建完整的模板系统与测试框架（gdUnit4），并通过输入映射与状态管理实现流畅的玩法体验。

- 支持平台：Windows、Linux、macOS
- 引擎版本：Godot 4.6
- 测试框架：gdUnit4
- 模块化设计：清晰的脚本与场景组织，便于扩展与定制
- **更新**：场景组织结构已重构，从独立的 Sample.tscn 迁移到新的 DefaultScene 结构，提供更清晰的示例
- **更新**：相机系统已从 ease_type 系统完全迁移到新的 tween_transition 和 tween_ease 属性系统
- **更新**：修复BaseFloor3对象的位置坐标Y坐标精度问题，提升场景布局精确度
- **更新**：关卡标题本地化优化，DefaultScene和Sample场景的levelTitle统一为"测试场景"，提供更描述性的中文标题

**章节来源**
- [README.md: 1-102:1-102](file://README.md#L1-L102)

## 项目结构
项目采用模板化与模块化结合的组织方式：
- #Template/：核心模板资源与脚本，包含场景、材质、音效、触发器与工具脚本
- Tests/：单元测试（gdUnit4）
- addons/gdUnit4/：测试插件与工具
- reports/：测试报告输出目录
- project.godot：项目配置（输入映射、层位、渲染等）
- 其他：README、贡献指南、导出预设等

```mermaid
graph TB
subgraph "项目根目录"
A["project.godot"]
B["README.md"]
C["CONTRIBUTING.md"]
D["export_presets.cfg"]
end
subgraph "模板系统 #Template/"
E["[Scripts]/GameManager.gd"]
F["[Scripts]/Level/MainLine.gd"]
G["[Scripts]/State.gd"]
H["[Scripts]/Level/RoadMaker.gd"]
I["[Scripts]/Level/gameui.gd"]
J["[Scripts]/Trigger/*.gd"]
K["[Scripts]/CameraScripts/*.gd"]
L["Scenes/DefaultScene/*"]
M["Scenes/Sample/*"]
N["Resources/*"]
O["CrownCheckPoint.tscn"]
P["MainLine.tscn"]
end
subgraph "测试系统 addons/gdUnit4/"
Q["plugin.gd"]
R["plugin.cfg"]
end
subgraph "测试用例 Tests/"
S["MainLine_test.gd"]
T["Crown_test.gd"]
end
A --> E
A --> F
A --> G
A --> H
A --> I
A --> J
A --> K
A --> L
A --> M
A --> N
O --> P
Q --> R
S --> F
T --> J
```

**图表来源**
- [project.godot: 1-88:1-88](file://project.godot#L1-L88)
- [plugin.gd: 1-102:1-102](file://addons/gdUnit4/plugin.gd#L1-L102)
- [plugin.cfg: 1-8:1-8](file://addons/gdUnit4/plugin.cfg#L1-L8)

**章节来源**
- [README.md: 52-61:52-61](file://README.md#L52-L61)
- [project.godot: 15-88:15-88](file://project.godot#L15-L88)

## 核心组件
- GameManager：负责相机、主线与动画起始时间计算等
- MainLine：角色主体，处理移动、转向、连线绘制、死亡与特效
- State：全局状态容器，保存相机检查点、动画时间、音乐检查点、通关状态等
- RoadMaker：根据主线位置动态生成道路网格
- gameui：结算界面与交互逻辑
- 触发器：Crown（皇冠）、Diamond（钻石）、Trigger（通用触发器）
- **相机系统**：CameraFollower（相机跟随器）、CameraTrigger（相机触发器）、CamShaker（相机震动器）

**章节来源**
- [GameManager.gd: 1-50:1-50](file://#Template/[Scripts]/GameManager.gd#L1-L50)
- [MainLine.gd: 1-218:1-218](file://#Template/[Scripts]/Level/MainLine.gd#L1-L218)
- [State.gd: 1-196:1-196](file://#Template/[Scripts]/State.gd#L1-L196)
- [RoadMaker.gd: 1-46:1-46](file://#Template/[Scripts]/Level/RoadMaker.gd#L1-L46)
- [gameui.gd: 1-74:1-74](file://#Template/[Scripts]/Level/gameui.gd#L1-L74)
- [Crown.gd: 1-21:1-21](file://#Template/[Scripts]/Trigger/Crown.gd#L1-L21)
- [Diamond.gd: 1-15:1-15](file://#Template/[Scripts]/Trigger/Diamond.gd#L1-L15)
- [Trigger.gd: 1-10:1-10](file://#Template/[Scripts]/Trigger/Trigger.gd#L1-L10)
- [CameraFollower.gd: 1-150:1-150](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L1-L150)
- [CameraTrigger.gd: 1-109:1-109](file://#Template/[Scripts]/CameraScripts/CameraTrigger.gd#L1-L109)
- [CamShaker.gd: 1-33:1-33](file://#Template/[Scripts]/CameraScripts/CamShaker.gd#L1-L33)

## 架构总览
整体架构围绕"状态驱动 + 事件触发"的模式构建：
- 输入事件通过 project.godot 的输入映射触发 MainLine 的转向逻辑
- MainLine 在物理帧中更新运动与连线，同时与 RoadMaker 协作生成道路
- 触发器（Crown/Diamond）通过 Area3D 与 MainLine 交互，更新 State
- GameManager 负责计算动画起始时间并与 MainLine 的动画同步
- gameui 基于 State 显示结算界面与交互
- **相机系统**：CameraFollower 提供平滑的相机跟随，CameraTrigger 处理相机参数的过渡动画，CamShaker 提供震动效果

```mermaid
graph TB
Input["输入映射<br/>project.godot"] --> ML["Level/MainLine.gd"]
ML --> RM["Level/RoadMaker.gd"]
ML --> ST["State.gd"]
ML --> GM["GameManager.gd"]
ML --> UI["Level/gameui.gd"]
subgraph "相机系统"
CF["CameraFollower.gd"]
CT["CameraTrigger.gd"]
CS["CamShaker.gd"]
end
CF --> CT
CF --> CS
GM --> CF
```

**图表来源**
- [project.godot: 43-71:43-71](file://project.godot#L43-L71)
- [MainLine.gd: 56-201:56-201](file://#Template/[Scripts]/Level/MainLine.gd#L56-L201)
- [RoadMaker.gd: 22-46:22-46](file://#Template/[Scripts]/Level/RoadMaker.gd#L22-L46)
- [GameManager.gd: 23-39:23-39](file://#Template/[Scripts]/GameManager.gd#L23-L39)
- [State.gd: 1-196:1-196](file://#Template/[Scripts]/State.gd#L1-L196)
- [Crown.gd: 16-42:16-42](file://#Template/[Scripts]/Trigger/Crown.gd#L16-L42)
- [Diamond.gd: 6-15:6-15](file://#Template/[Scripts]/Trigger/Diamond.gd#L6-L15)
- [Trigger.gd: 8-10:8-10](file://#Template/[Scripts]/Trigger/Trigger.gd#L8-L10)
- [CameraFollower.gd: 1-150:1-150](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L1-L150)
- [CameraTrigger.gd: 1-109:1-109](file://#Template/[Scripts]/CameraScripts/CameraTrigger.gd#L1-L109)
- [CamShaker.gd: 1-33:1-33](file://#Template/[Scripts]/CameraScripts/CamShaker.gd#L1-L33)

## 详细组件分析

### GameManager 组件分析
职责与行为
- 暴露相机、主线引用与系数因子
- 提供原点位置设置与跳转功能
- 计算动画起始时间：基于主线当前位置与原点的2D距离、速度与系数

复杂度与性能
- 计算为 O(1)，仅涉及向量距离与除法运算
- 速度为零时返回 0，避免无效时间

```mermaid
classDiagram
class GameManager {
+Camera3D Camera
+CharacterBody3D Mainline
+float factor
+Vector3 origin_pos
+calculate_anim_start_time() float
+get_camera_follower() Node3D
}
```

**图表来源**
- [GameManager.gd: 5-50:5-50](file://#Template/[Scripts]/GameManager.gd#L5-L50)

**章节来源**
- [GameManager.gd: 11-39:11-39](file://#Template/[Scripts]/GameManager.gd#L11-L39)

### MainLine 组件分析
职责与行为
- 物理移动：重力、地板检测、飞行模式、穿墙模式
- 转向与动画：响应输入事件，播放动画并同步音乐时间
- 连线绘制：每步生成线段，地面阶段同步地面段高度
- 死亡与粒子：碰撞墙体或触发器时死亡，生成粒子效果
- 状态持久化：在重载时保存 transform、动画时间、音乐检查点等

关键流程（转向与动画同步）
```mermaid
sequenceDiagram
participant P as "玩家输入"
participant ML as "Level/MainLine"
participant GM as "GameManager"
participant AN as "AnimationPlayer"
participant MP as "MusicPlayer"
P->>ML : "turn" 输入
ML->>ML : turn()
ML->>AN : play("level")
ML->>GM : calculate_anim_start_time()
GM-->>ML : 返回动画起始时间
ML->>AN : seek(anim_time)
ML->>MP : 播放音乐(基于检查点或动画时间)
ML->>ML : 更新速度与旋转
ML->>ML : new_line() 生成线段
```

**图表来源**
- [MainLine.gd: 176-201:176-201](file://#Template/[Scripts]/Level/MainLine.gd#L176-L201)
- [GameManager.gd: 33-50:33-50](file://#Template/[Scripts]/GameManager.gd#L33-L50)

**章节来源**
- [MainLine.gd: 56-121:56-121](file://#Template/[Scripts]/Level/MainLine.gd#L56-L121)
- [MainLine.gd: 147-169:147-169](file://#Template/[Scripts]/Level/MainLine.gd#L147-L169)
- [MainLine.gd: 212-248:212-248](file://#Template/[Scripts]/Level/MainLine.gd#L212-L248)

### State 组件分析
职责与行为
- 存储全局状态：相机检查点、动画时间、音乐检查点、通关状态、分数等
- 与触发器交互：记录收集的皇冠数量与标签，设置检查点标志

```mermaid
classDiagram
class State {
+Transform main_line_transform
+Vector3 revive_position
+bool is_turn
+float anim_time
+float music_checkpoint_time
+bool is_end
+int percent
+int line_crossing_crown
+int crown
+int diamond
+int[] crowns
+Dictionary camera_checkpoint
+Dictionary main_line_data
+bool is_relive
+save_checkpoint() void
+load_checkpoint_to_main_line() void
+load_runtime_to_main_line() void
+load_to_camera_follower() void
+reset_to_defaults() void
}
```

**图表来源**
- [State.gd: 3-196:3-196](file://#Template/[Scripts]/State.gd#L3-L196)

**章节来源**
- [State.gd: 1-196:1-196](file://#Template/[Scripts]/State.gd#L1-L196)

### RoadMaker 组件分析
职责与行为
- 监听 MainLine 的 new_line1 与 on_sky 信号
- 动态生成道路网格，随主线移动更新位置与尺寸
- 提供保存功能，将生成的道路打包为场景资源

```mermaid
flowchart TD
Start(["初始化"]) --> Connect["连接信号<br/>new_line1/on_sky"]
Connect --> Sky{"离地?"}
Sky --> |否| NewRoad["new_road()<br/>实例化道路网格"]
Sky --> |是| Clear["road=null"]
NewRoad --> Update["物理帧更新<br/>位置与缩放"]
Update --> Save{"保存按键?"}
Save --> |是| Pack["打包并保存为场景"]
Save --> |否| Wait["等待下一步"]
Clear --> Wait
```

**图表来源**
- [RoadMaker.gd: 12-46:12-46](file://#Template/[Scripts]/Level/RoadMaker.gd#L12-L46)

**章节来源**
- [RoadMaker.gd: 22-46:22-46](file://#Template/[Scripts]/Level/RoadMaker.gd#L22-L46)

### 触发器组件分析
- Crown（皇冠）：旋转展示，进入时增加 State.crown，记录相机检查点与动画时间，设置标签检查点
- Diamond（钻石）：旋转展示，进入时增加 State.diamond，播放粒子与动画后销毁
- Trigger（通用触发器）：发出 hit_the_line 信号

```mermaid
classDiagram
class Crown {
+float speed
+int tag
+_get_camera_follower() Node3D
+_process(delta) void
+_on_Crown_body_entered(main_line) void
}
class Diamond {
+float speed
+_on_Diamond_body_entered(body) void
+_process(delta) void
}
class Trigger {
+hit_the_line
+_on_triggered(body) void
}
Crown --> State : "更新状态"
Diamond --> State : "更新状态"
```

**图表来源**
- [Crown.gd: 3-21:3-21](file://#Template/[Scripts]/Trigger/Crown.gd#L3-L21)
- [Diamond.gd: 4-15:4-15](file://#Template/[Scripts]/Trigger/Diamond.gd#L4-L15)
- [Trigger.gd: 6-10:6-10](file://#Template/[Scripts]/Trigger/Trigger.gd#L6-L10)

**章节来源**
- [Crown.gd: 16-42:16-42](file://#Template/[Scripts]/Trigger/Crown.gd#L16-L42)
- [Diamond.gd: 6-15:6-15](file://#Template/[Scripts]/Trigger/Diamond.gd#L6-L15)
- [Trigger.gd: 8-10:8-10](file://#Template/[Scripts]/Trigger/Trigger.gd#L8-L10)

### 测试框架与用例分析
- gdUnit4 插件：在编辑器中安装控制台与检查器，支持无头模式运行测试
- 测试用例：
  - MainLine_test.gd：验证 MainLine 场景存在、继承关系、属性与方法
  - Crown_test.gd：验证 Crown 脚本与场景存在、继承关系、属性、状态更新与方法存在性

```mermaid
sequenceDiagram
participant Editor as "Godot 编辑器"
participant Plugin as "gdUnit4 插件"
participant Runner as "测试运行器"
participant Suite1 as "MainLine_test.gd"
participant Suite2 as "Crown_test.gd"
Editor->>Plugin : 加载插件
Plugin-->>Editor : 显示控制台与检查器
Editor->>Runner : 运行测试
Runner->>Suite1 : 执行用例
Runner->>Suite2 : 执行用例
Suite1-->>Runner : 报告结果
Suite2-->>Runner : 报告结果
Runner-->>Editor : 输出报告
```

**图表来源**
- [plugin.gd: 11-72:11-72](file://addons/gdUnit4/plugin.gd#L11-L72)
- [plugin.cfg: 1-8:1-8](file://addons/gdUnit4/plugin.cfg#L1-L8)
- [MainLine_test.gd: 1-250:1-250](file://Tests/MainLine_test.gd#L1-L250)
- [Crown_test.gd: 1-178:1-178](file://Tests/Crown_test.gd#L1-L178)

**章节来源**
- [plugin.gd: 11-72:11-72](file://addons/gdUnit4/plugin.gd#L11-L72)
- [MainLine_test.gd: 1-250:1-250](file://Tests/MainLine_test.gd#L1-L250)
- [Crown_test.gd: 1-178:1-178](file://Tests/Crown_test.gd#L1-L178)

## 场景组织结构

### DefaultScene 结构重组
**更新**：场景组织结构已完全重构，从独立的 Sample.tscn 迁移到新的 DefaultScene 结构

重构前的问题
- Sample.tscn 作为独立场景文件，与其他模板资源耦合度较高
- 场景配置分散在多个文件中，维护困难
- 相机触发器配置不够直观，需要手动调整多个属性

重构后的结构
- **DefaultScene/**：新的场景组织结构，包含 Default.tscn 和 Default.tres
- **Default.tscn**：核心场景文件，包含完整的相机系统配置
- **Default.tres**：场景数据资源，定义关卡标题、音频、颜色等配置
- **Sample/**：保留的示例场景，包含更多触发器和装饰元素

新的场景文件结构
```mermaid
graph TB
subgraph "DefaultScene 结构"
DS["DefaultScene/"]
DT["Default.tscn<br/>核心场景"]
DR["Default.tres<br/>场景数据资源"]
end
subgraph "Sample 结构"
SS["Sample/"]
ST["Sample.tscn<br/>示例场景"]
SR["Sample.tres<br/>示例场景数据"]
end
DS --> DT
DS --> DR
SS --> ST
SS --> SR
```

**图表来源**
- [Default.tscn: 1-82:1-82](file://#Template/[Scenes]/DefaultScene/Default.tscn#L1-L82)
- [Default.tres: 1-35:1-35](file://#Template/[Scenes]/DefaultScene/Default.tres#L1-L35)
- [Sample.tscn: 1-141:1-141](file://#Template/[Scenes]/Sample/Sample.tscn#L1-L141)
- [Sample.tres: 1-35:1-35](file://#Template/[Scenes]/Sample/Sample.tres#L1-L35)

重构优势
- **模块化组织**：场景文件与数据资源分离，便于维护和扩展
- **清晰的层次结构**：DefaultScene 提供简洁的核心示例，Sample 提供丰富的演示
- **更好的可扩展性**：新的结构支持快速创建新的关卡变体
- **统一的配置管理**：LevelData 资源提供标准化的关卡配置接口

**章节来源**
- [Default.tscn: 1-82:1-82](file://#Template/[Scenes]/DefaultScene/Default.tscn#L1-L82)
- [Default.tres: 1-35:1-35](file://#Template/[Scenes]/DefaultScene/Default.tres#L1-L35)
- [Sample.tscn: 1-141:1-141](file://#Template/[Scenes]/Sample/Sample.tscn#L1-L141)
- [Sample.tres: 1-35:1-35](file://#Template/[Scenes]/Sample/Sample.tres#L1-L35)

### 场景文件结构对比
- **Default.tscn**：核心场景，包含相机系统、玩家角色、灯光、UI 等基本元素
- **Sample.tscn**：演示场景，包含多个触发器、装饰物、更复杂的场景布局
- **MainLine.tscn**：主角色场景，包含 Player 的完整配置
- **CrownCheckPoint.tscn**：Crown 检查点场景模板

```mermaid
graph TB
subgraph "默认场景 DefaultScene"
DT["Default.tscn<br/>核心场景"]
DR["Default.tres<br/>场景数据"]
end
subgraph "示例场景 Sample"
ST["Sample.tscn<br/>演示场景"]
SR["Sample.tres<br/>场景数据"]
end
subgraph "其他场景"
SC["Scene.tscn<br/>基础场景"]
ML["MainLine.tscn<br/>主角色场景"]
CC["CrownCheckPoint.tscn<br/>检查点场景"]
end
DT --> DR
ST --> SR
SC --> ML
SC --> CC
```

**图表来源**
- [Default.tscn: 1-82:1-82](file://#Template/[Scenes]/DefaultScene/Default.tscn#L1-L82)
- [Sample.tscn: 1-141:1-141](file://#Template/[Scenes]/Sample/Sample.tscn#L1-L141)
- [Scene.tscn: 1-85:1-85](file://#Template/[Scenes]/Scene.tscn#L1-L85)
- [MainLine.tscn: 1-72:1-72](file://#Template/MainLine.tscn#L1-L72)
- [CrownCheckPoint.tscn: 1-104:1-104](file://#Template/CrownCheckPoint.tscn#L1-L104)

**章节来源**
- [Default.tscn: 1-82:1-82](file://#Template/[Scenes]/DefaultScene/Default.tscn#L1-L82)
- [Sample.tscn: 1-141:1-141](file://#Template/[Scenes]/Sample/Sample.tscn#L1-L141)
- [Scene.tscn: 1-85:1-85](file://#Template/[Scenes]/Scene.tscn#L1-L85)
- [MainLine.tscn: 1-72:1-72](file://#Template/MainLine.tscn#L1-L72)
- [CrownCheckPoint.tscn: 1-104:1-104](file://#Template/CrownCheckPoint.tscn#L1-L104)

### 场景配置与本地化优化

#### 关卡标题本地化优化
**更新**：关卡标题已统一为"测试场景"，提供更描述性的中文标题

DefaultScene 和 Sample 场景中的关卡配置
- **levelTitleKey**：保持为 "Default"，用于国际化标识
- **levelTitle**：统一设置为 "测试场景"，提供更描述性的中文标题
- **authors**：作者信息保持不变

```mermaid
flowchart TD
Start(["关卡标题配置"]) --> Config["Default.tres & Sample.tres 配置"]
Config --> Key["levelTitleKey<br/>\"Default\""]
Config --> Title["levelTitle<br/>\"测试场景\""]
Config --> Authors["authors<br/>作者信息"]
Title --> Localize["本地化显示"]
Localize --> Describe["提供更描述性标题"]
```

**图表来源**
- [Default.tres: 30-35:30-35](file://#Template/[Scenes]/DefaultScene/Default.tres#L30-L35)
- [Sample.tres: 28-35:28-35](file://#Template/[Scenes]/Sample/Sample.tres#L28-L35)

**章节来源**
- [Default.tres: 30-35:30-35](file://#Template/[Scenes]/DefaultScene/Default.tres#L30-L35)
- [Sample.tres: 28-35:28-35](file://#Template/[Scenes]/Sample/Sample.tres#L28-L35)

#### 场景布局精确度优化

**更新**：修复BaseFloor3对象的位置坐标Y坐标精度问题

Sample 场景中的BaseFloor3配置
- **transform**：包含精确的位置变换矩阵
- **Y坐标**：从 2.4076805 调整为 2.3337727，提升场景布局精确度
- **其他坐标**：X=75.14364, Z=63.830173 保持不变

```mermaid
flowchart TD
Start(["场景布局配置"]) --> Floor3["BaseFloor3 配置"]
Floor3 --> Position["位置变换"]
Position --> X["X: 75.14364"]
Position --> Y["Y: 2.3337727"]
Position --> Z["Z: 63.830173"]
Y --> Precision["精度优化"]
Precision --> Improve["提升布局精确度"]
```

**图表来源**
- [Sample.tscn: 124-127:124-127](file://#Template/[Scenes]/Sample/Sample.tscn#L124-L127)

**章节来源**
- [Sample.tscn: 124-127:124-127](file://#Template/[Scenes]/Sample/Sample.tscn#L124-L127)

## 相机系统详解

### CameraFollower 相机跟随器
**更新**：相机跟随器现在支持新的 tween_transition 和 tween_ease 属性系统

职责与行为
- 平滑跟随玩家角色，提供自然的相机移动体验
- 支持多种相机参数的 Tween 动画：位置、旋转、距离、跟随速度
- 提供相机检查点功能，支持状态恢复
- 内置 Lerp 过渡系统，支持最短角路径旋转

关键特性
- **Tween 系统**：使用 `tween_transition` 和 `tween_ease` 属性控制动画曲线
- **多属性管理**：统一管理相机位置、旋转、距离和速度的 Tween 实例
- **状态备份**：支持相机参数的备份与恢复
- **实时中断**：支持停止正在进行的 Tween 动画

```mermaid
classDiagram
class CameraFollower {
+NodePath player
+Vector3 add_position
+Vector3 rotation_offset
+float distance_from_object
+float follow_speed
+bool following
+Tween.TransitionType tween_transition
+Tween.EaseType tween_ease
+tween_to_position(Vector3, float, Tween.EaseType, Tween.TransitionType) void
+tween_to_rotation(Vector3, float, Tween.EaseType, Tween.TransitionType) void
+tween_to_distance(float, float, Tween.EaseType, Tween.TransitionType) void
+tween_to_speed(float, float, Tween.EaseType, Tween.TransitionType) void
+kill_tweens() void
+pick() void
+revive() void
}
```

**图表来源**
- [CameraFollower.gd: 1-150:1-150](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L1-L150)

**章节来源**
- [CameraFollower.gd: 1-150:1-150](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L1-L150)

### CameraTrigger 相机触发器
**更新**：相机触发器已从 ease_type 系统完全迁移到新的 tween_transition 和 tween_ease 属性系统

职责与行为
- 检测玩家进入触发区域时执行相机参数变更
- 支持基于时间或即时触发的相机切换
- 使用 Tween 系统平滑过渡相机参数

新属性系统
- `tween_transition`：控制动画的过渡类型（如 Sine、Linear、Quad 等）
- `tween_ease`：控制动画的缓动类型（如 In、Out、InOut 等）
- `need_time`：动画持续时间

```mermaid
classDiagram
class CameraTrigger {
+bool active_position
+Vector3 new_add_position
+bool active_rotate
+Vector3 new_rotation
+bool active_distance
+float new_distance
+bool active_speed
+float new_follow_speed
+RotateMode rotate_mode
+Tween.TransitionType tween_transition
+Tween.EaseType tween_ease
+float need_time
+bool use_time
+float trigger_time
+_trigger() void
}
```

**图表来源**
- [CameraTrigger.gd: 1-109:1-109](file://#Template/[Scripts]/CameraScripts/CameraTrigger.gd#L1-L109)

**章节来源**
- [CameraTrigger.gd: 1-109:1-109](file://#Template/[Scripts]/CameraScripts/CameraTrigger.gd#L1-L109)

### CamShaker 相机震动器
职责与行为
- 提供相机震动效果，增强游戏的反馈体验
- 支持自定义震动强度和持续时间
- 基于随机偏移实现自然的震动效果

```mermaid
classDiagram
class CamShaker {
+Node3D camera_parent
+float shake_intensity
+float shake_duration
+float shake_timer
+Vector3 original_position
+_on_body_entered(Node3D) void
}
```

**图表来源**
- [CamShaker.gd: 1-33:1-33](file://#Template/[Scripts]/CameraScripts/CamShaker.gd#L1-L33)

**章节来源**
- [CamShaker.gd: 1-33:1-33](file://#Template/[Scripts]/CameraScripts/CamShaker.gd#L1-L33)

### DefaultScene 中的相机配置
**更新**：DefaultScene 场景中相机触发器已配置新的 Tween 属性系统

DefaultScene 中的相机触发器配置
- **Trigger2**：配置了 `tween_transition = 1`（Sine）和 `tween_ease = 2`（InOut）
- **Trigger3**：同样配置了 `tween_transition = 1`（Sine）和 `tween_ease = 2`（InOut）
- 所有触发器都设置了 `need_time = 1.0` 或 `need_time = 2.0` 的动画持续时间

```mermaid
flowchart TD
Start(["相机触发器配置"]) --> Config["配置 Tween 属性"]
Config --> Transition["tween_transition<br/>1 = Sine"]
Config --> Ease["tween_ease<br/>2 = InOut"]
Config --> Duration["need_time<br/>1.0/2.0秒"]
Config --> Apply["应用到相机跟随器"]
Config --> Smooth["平滑过渡相机参数"]
```

**图表来源**
- [Sample.tscn: 90-110:90-110](file://#Template/[Scenes]/Sample/Sample.tscn#L90-L110)

**章节来源**
- [Sample.tscn: 90-110:90-110](file://#Template/[Scenes]/Sample/Sample.tscn#L90-L110)

## 依赖关系分析
- 项目配置依赖：project.godot 定义输入映射、层位、渲染与插件启用
- 运行时依赖：MainLine 依赖 State、RoadMaker、GameManager；触发器依赖 State
- 测试依赖：gdUnit4 插件与测试套件
- **相机系统依赖**：CameraTrigger 依赖 CameraFollower，GameManager 提供相机跟随器访问
- **场景组织依赖**：DefaultScene 和 Sample 场景的正确配置依赖于新的结构
- **本地化依赖**：level_patch_meta.tres 提供关卡标题的本地化配置

```mermaid
graph LR
PG["project.godot"] --> ML["Level/MainLine.gd"]
PG --> ST["State.gd"]
PG --> RM["Level/RoadMaker.gd"]
PG --> GM["GameManager.gd"]
ML --> ST
ML --> RM
ML --> GM
CR["Crown.gd"] --> ST
DM["Diamond.gd"] --> ST
TR["Trigger.gd"] --> ML
CF["CameraFollower.gd"] --> GM
CT["CameraTrigger.gd"] --> CF
CS["CamShaker.gd"] --> CF
DS["DefaultScene/Default.tscn"] --> CF
DS --> CT
DS --> CS
SS["Sample/Sample.tscn"] --> CF
SS --> CT
SS --> CS
LM["level_patch_meta.tres"] --> DS
LM --> SS
```

**图表来源**
- [project.godot: 22-88:22-88](file://project.godot#L22-L88)
- [MainLine.gd: 1-218:1-218](file://#Template/[Scripts]/Level/MainLine.gd#L1-L218)
- [RoadMaker.gd: 1-46:1-46](file://#Template/[Scripts]/Level/RoadMaker.gd#L1-L46)
- [GameManager.gd: 1-50:1-50](file://#Template/[Scripts]/GameManager.gd#L1-L50)
- [Crown.gd: 1-21:1-21](file://#Template/[Scripts]/Trigger/Crown.gd#L1-L21)
- [Diamond.gd: 1-15:1-15](file://#Template/[Scripts]/Trigger/Diamond.gd#L1-L15)
- [Trigger.gd: 1-10:1-10](file://#Template/[Scripts]/Trigger/Trigger.gd#L1-L10)
- [CameraFollower.gd: 1-150:1-150](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L1-L150)
- [CameraTrigger.gd: 1-109:1-109](file://#Template/[Scripts]/CameraScripts/CameraTrigger.gd#L1-L109)
- [CamShaker.gd: 1-33:1-33](file://#Template/[Scripts]/CameraScripts/CamShaker.gd#L1-L33)
- [Default.tscn: 1-82:1-82](file://#Template/[Scenes]/DefaultScene/Default.tscn#L1-L82)
- [Sample.tscn: 1-141:1-141](file://#Template/[Scenes]/Sample/Sample.tscn#L1-L141)
- [level_patch_meta.tres: 13-14:13-14](file://#Template/[Resources]/level_patch_meta.tres#L13-L14)

**章节来源**
- [project.godot: 22-88:22-88](file://project.godot#L22-L88)

## 性能考量
- 物理与渲染
  - 3D 物理在独立线程运行，使用 Jolt 物理引擎，有助于提升性能
  - 移动端渲染方法设为 mobile，纹理 VRAM 压缩开启，有利于移动端表现
- 动画与音乐同步
  - 使用音画同步策略，减少视觉与音频不同步带来的额外计算
- 线段与网格
  - 线段与道路网格按需生成与更新，避免不必要的对象创建
- 测试环境
  - 无头模式运行测试，减少图形与音频开销
- **相机系统性能**
  - Tween 动画使用 Godot 内置优化，避免手动插值计算
  - 相机跟随使用球面插值（slerp），提供更自然的过渡效果
- **场景组织性能**
  - 重构后的 DefaultScene 结构减少了场景文件的复杂度，提高了加载效率
  - 分离的场景数据资源支持更灵活的资源管理
  - 新的结构提高了代码的可维护性和性能
- **本地化性能优化**
  - 关卡标题统一为"测试场景"，减少了字符串处理开销
  - BaseFloor3坐标精度优化提升了渲染性能和视觉一致性

**章节来源**
- [project.godot: 78-88:78-88](file://project.godot#L78-L88)
- [MainLine.gd: 74-76:74-76](file://#Template/[Scripts]/Level/MainLine.gd#L74-L76)
- [CameraFollower.gd: 54-62:54-62](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L54-L62)

## 故障排除指南
- 插件警告与兼容性
  - 若出现 GDScript 推断声明警告，需在项目设置中排除插件路径或关闭相关警告
  - 插件要求 Godot 4.5+，低版本会提示无法加载
- 输入映射问题
  - 检查 project.godot 的 input 节是否正确配置 turn/retry/save/reload/savetaper
- 动画与音乐不同步
  - 确认 MainLine 的音乐播放与 AnimationPlayer 的 seek 时间一致
- 线段未生成或消失
  - 确认 MainLine 已连接 new_line1 信号，且 RoadMaker 已添加到场景树
- 测试运行异常
  - 使用无头模式运行测试，确认 gdUnit4 插件已启用
- **相机系统问题**
  - **Tween 属性配置错误**：检查相机触发器的 `tween_transition` 和 `tween_ease` 是否为有效的 Tween 枚举值
  - **相机跟随异常**：确认 CameraFollower 的 `player` 属性指向正确的玩家节点
  - **动画不生效**：检查相机触发器是否正确调用了 `kill_tweens()` 来中断现有动画
- **场景组织问题**
  - **DefaultScene 结构错误**：检查 DefaultScene 目录下的文件组织，确保 Default.tscn 和 Default.tres 正确关联
  - **Sample 场景配置问题**：验证 Sample.tscn 中的相机触发器配置，确保 Tween 属性正确设置
  - **资源路径问题**：确认场景文件中的资源路径指向正确的 DefaultScene 或 Sample 目录
- **本地化问题**
  - **关卡标题显示异常**：检查 Default.tres 和 Sample.tres 中的 `levelTitle` 字段是否正确设置为 "测试场景"
  - **中文字符编码问题**：确认 level_patch_meta.tres 中的 `chinese_name` 字段编码正确
- **场景布局问题**
  - **BaseFloor3位置偏差**：检查 Sample.tscn 中 BaseFloor3 的 Y坐标是否为 2.3337727
  - **场景坐标精度问题**：验证其他场景元素的坐标精度，确保视觉一致性

**章节来源**
- [plugin.gd: 11-38:11-38](file://addons/gdUnit4/plugin.gd#L11-L38)
- [project.godot: 43-71:43-71](file://project.godot#L43-L71)
- [MainLine.gd: 147-169:147-169](file://#Template/[Scripts]/Level/MainLine.gd#L147-L169)
- [RoadMaker.gd: 12-21:12-21](file://#Template/[Scripts]/Level/RoadMaker.gd#L12-L21)
- [CameraTrigger.gd: 54-75:54-75](file://#Template/[Scripts]/CameraScripts/CameraTrigger.gd#L54-L75)
- [CameraFollower.gd: 86-132:86-132](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L86-L132)
- [Default.tscn: 1-82:1-82](file://#Template/[Scenes]/DefaultScene/Default.tscn#L1-L82)
- [Sample.tscn: 1-141:1-141](file://#Template/[Scenes]/Sample/Sample.tscn#L1-L141)
- [Default.tres: 30-35:30-35](file://#Template/[Scenes]/DefaultScene/Default.tres#L30-L35)
- [level_patch_meta.tres: 13-14:13-14](file://#Template/[Resources]/level_patch_meta.tres#L13-L14)

## 结论
本模板以模块化与测试驱动为核心，提供了 Dancing Line 的完整实现骨架。通过清晰的状态管理、事件驱动的触发器系统、高效的物理渲染配置以及全新的相机 Tween 动画系统，能够快速搭建关卡并进行功能扩展。gdUnit4 的集成保证了代码质量与可维护性，适合团队协作与持续交付。

**更新总结**：
- 场景组织结构已完全重构，从独立的 Sample.tscn 迁移到新的 DefaultScene 结构，提供更清晰的示例
- 相机系统已完全迁移到新的 Tween 属性系统
- CameraTrigger 和 CameraFollower 现在使用 `tween_transition` 和 `tween_ease` 替代旧的 `ease_type` 系统
- DefaultScene/Default.tscn 和 DefaultScene/Default.tres 提供了标准化的核心示例
- Sample/Sample.tscn 保留了丰富的演示场景，包含更多触发器和装饰元素
- 新的系统提供了更灵活和直观的相机动画控制
- 重构后的场景结构提高了代码的可维护性和性能
- **新增**：修复BaseFloor3对象的位置坐标Y坐标精度问题，提升场景布局精确度
- **新增**：关卡标题本地化优化，DefaultScene和Sample场景的levelTitle统一为"测试场景"，提供更描述性的中文标题

## 附录
- 快速开始
  - 克隆仓库并在 Godot 4.6 中打开项目
  - 运行 DefaultScene/Default.tscn 或使用 Main.tscn 启动
- 输入控制
  - 转向：鼠标左键或空格
  - 重试：R
  - 保存：S
  - 重载：Q
  - 保存锥体：W
- 测试运行
  - 无头模式：godot --headless --run-tests
  - 编辑器中：打开底部 gdUnit4 面板
- **相机系统配置**
  - **Tween 类型**：TRANS_LINEAR、TRANS_SINE、TRANS_QUAD、TRANS_CUBIC 等
  - **缓动类型**：EASE_IN、EASE_OUT、EASE_IN_OUT
  - **动画持续时间**：建议 1.0-2.0 秒之间
  - **相机触发器使用**：支持即时触发和基于时间的触发
- **场景组织最佳实践**
  - **DefaultScene 结构**：使用 DefaultScene/Default.tscn 作为核心示例，DefaultScene/Default.tres 作为场景数据
  - **Sample 场景**：使用 Sample/Sample.tscn 作为演示场景，包含更多触发器和装饰元素
  - **资源管理**：将场景数据与场景文件分离，便于维护和扩展
  - **相机系统**：在 DefaultScene 中使用新的 Tween 属性系统，在 Sample 中提供更复杂的配置示例
- **本地化配置**
  - **关卡标题**：Default.tres 和 Sample.tres 中的 `levelTitle` 设置为 "测试场景"
  - **中文名称**：level_patch_meta.tres 中的 `chinese_name` 设置为 "测试"
  - **国际化标识**：`levelTitleKey` 保持为 "Default"
- **场景布局优化**
  - **BaseFloor3坐标**：Y坐标精确调整为 2.3337727
  - **视觉一致性**：确保所有场景元素的坐标精度一致
  - **渲染性能**：优化的坐标精度提升渲染性能和视觉效果

**章节来源**
- [README.md: 18-51:18-51](file://README.md#L18-L51)
- [CONTRIBUTING.md: 41-59:41-59](file://CONTRIBUTING.md#L41-L59)
- [CameraTrigger.gd: 11-12:11-12](file://#Template/[Scripts]/CameraScripts/CameraTrigger.gd#L11-L12)
- [CameraFollower.gd: 9-10:9-10](file://#Template/[Scripts]/CameraScripts/CameraFollower.gd#L9-L10)
- [Default.tscn: 98-116:98-116](file://#Template/[Scenes]/DefaultScene/Default.tscn#L98-L116)
- [Sample.tscn: 90-110:90-110](file://#Template/[Scenes]/Sample/Sample.tscn#L90-L110)
- [Default.tres: 30-35:30-35](file://#Template/[Scenes]/DefaultScene/Default.tres#L30-L35)
- [level_patch_meta.tres: 13-14:13-14](file://#Template/[Resources]/level_patch_meta.tres#L13-L14)
- [Sample.tscn: 124-127:124-127](file://#Template/[Scenes]/Sample/Sample.tscn#L124-L127)
...
**最新更新** 系统现已增强自动清理功能，增加了基于定时器的收缩动画，能够自动管理粒子生命周期，有效防止内存泄漏。
...
- [MainLine.gd:30](file://#Template/[Scripts]/Level/MainLine.gd#L30)
...
| **自动清理** | **新增** 基于定时器的收缩动画 | 延迟2秒开始<br/>1秒内缩放至0 |

### 自动清理机制

**新增功能** 死亡粒子实体现在包含完整的自动清理生命周期管理：

```mermaid
stateDiagram-v2
[*] --> 初始化
初始化 --> 等待定时器 : _start_shrink_timer()
等待定时器 --> 开始收缩 : 定时器超时
开始收缩 --> 缩放动画 : 创建Tween
缩放动画 --> 清理完成 : 动画结束
清理完成 --> [*] : queue_free()
...
**图表来源**
- [death_particle.gd:10-28](file://#Template/[Scripts]/Level/death_particle.gd#L10-L28)
...
Particle->>Timer : _start_shrink_timer()
Particle->>Physics : 应用冲量
Particle->>Physics : 应用扭矩
end
Timer-->>Particle : 定时器超时
Timer-->>Particle : _start_shrink()
...
**图表来源**
- [MainLine.gd:195-224](file://#Template/[Scripts]/Level/MainLine.gd#L195-L224)
...
DeathParticle --> TimerSystem
TimerSystem --> CleanupManager
PhysicsEngine --> CollisionSystem
VisualEffects --> DeathParticle
```

**更新** 新增的定时器系统和清理管理器确保了粒子的自动生命周期管理。
...
flowchart TD
Start([粒子创建]) --> Init[初始化]
Init --> StartTimer[_start_shrink_timer()]
StartTimer --> Delay{等待shrink_delay秒}
Delay --> |定时器超时| StartShrink[_start_shrink()]
StartShrink --> CreateTween[创建Tween]
CreateTween --> ScaleDown[缩放到Vector3.ZERO]
ScaleDown --> OnFinished[_on_shrink_finished]
OnFinished --> QueueFree[queue_free()]
QueueFree --> End([清理完成])
subgraph "配置参数"
shrink_delay[延迟时间: 2.0秒]
shrink_duration[持续时间: 1.0秒]
end
StartTimer -.-> shrink_delay
StartShrink -.-> shrink_duration
```

**图表来源**
- [death_particle.gd:13-28](file://#Template/[Scripts]/Level/death_particle.gd#L13-L28)
...
PhysicsSim --> AutoClean[自动清理启动]
AutoClean --> End([粒子生成完成])
```

**更新** 现在通过MainLine.tscn场景文件中的ExtResource引用DeathParticle.tscn，确保了正确的资源定位。

**图表来源**
- [MainLine.gd:210-224](file://#Template/[Scripts]/Level/MainLine.gd#L210-L224)
- [MainLine.tscn:40](file://#Template/MainLine.tscn#L40)
...
**章节来源**
- [MainLine.gd:210-227](file://#Template/[Scripts]/Level/MainLine.gd#L210-L227)
...
**章节来源**
- [MainLine.gd:200-201](file://#Template/[Scripts]/Level/MainLine.gd#L200-L201)
...
**更新** 新增的定时器系统依赖关系，确保了自动清理功能的正常运行。
...
PhysicsUpdate --> TimerSetup[定时器设置]
TimerSetup --> ShrinkAnimation[收缩动画]
ShrinkAnimation --> AutoCleanup[自动清理]
AutoCleanup --> VisualFeedback[视觉反馈]
...
**更新** 新增的定时器设置和收缩动画阶段，确保了粒子的完整生命周期管理。
...
5. **新增**：验证death_particle.gd中的定时器初始化
...
4. **新增**：检查shrink_delay和shrink_duration参数
...
- **状态变更**：is_live状态切换
- **定时器回调**：_start_shrink_timer()、_on_shrink_finished()
...
5. **生命周期管理**：**新增** 进一步优化自动清理机制

**更新** 新的资源组织结构和自动清理功能为系统的长期维护和发展奠定了更好的基础，确保了资源管理的规范性和可扩展性。

该系统为类似的游戏特效开发提供了优秀的参考范例，展示了如何在实际项目中平衡技术实现、性能考虑和用户体验。新增的自动清理功能特别适用于需要长时间运行的游戏场景，确保了系统的稳定性和可靠性。
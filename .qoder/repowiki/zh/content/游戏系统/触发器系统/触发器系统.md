...
## 更新摘要
**变更内容**
- **更新**：ChangeSpeedTrigger.gd 移除了即时速度向量更新逻辑，现在仅设置基础速度属性
- **更新**：PreEnding.gd 改进了定时机制，使用 call_deferred() 确保正确的序列执行顺序
...
- **更新**：移除了即时速度向量更新逻辑
- 行为逻辑
  - **更新**：现在仅设置 body.speed = new_speed，不再同步更新移动向量 v
...
**图表来源**
- [ChangeSpeedTrigger.gd:8-12](file://#Template/[Scripts]/Trigger/ChangeSpeedTrigger.gd#L8-L12)

**章节来源**
- [ChangeSpeedTrigger.gd:1-12](file://#Template/[Scripts]/Trigger/ChangeSpeedTrigger.gd#L1-L12)
...
- 参数配置
  - Offset：相机偏移向量
- **更新**：改进了定时机制，使用 call_deferred() 确保正确的序列执行
- 行为逻辑
  - 获取相机跟随器并进行平滑过渡
  - 播放 jinzita 动画
  - 使角色朝向触发器方向
...
```mermaid
flowchart TD
Start(["角色进入触发区域"]) --> GetCamera["获取相机跟随器"]
...
- 行为异常
  ...
  - ChangeSpeedTrigger：确认目标节点具备 speed 字段，**更新**：现在仅设置速度属性，不直接更新向量
  - ChangeTurn：确认目标节点具备 is_turn 字段
  - **更新**：Ending：确认仅设置 State.is_end 标志
  - **新增**：PreEnding：确认 GameManager 中存在相机跟随器，**更新**：检查 call_deferred 序列执行
- 收集型触发器
...
- **更新**：架构简化后的故障排查
  ...
  - **更新**：验证 ChangeSpeedTrigger 的速度设置逻辑
...
- [trigger.tscn:23](file://#Template/trigger.tscn#L23](file://#Template/trigger.tscn#L23))
...
- ChangeSpeedTrigger 移除了即时速度向量更新逻辑，现在仅设置基础速度属性，依赖 Player.gd 的 turn() 方法在转向时自动应用新的速度值
- PreEnding 触发器使用 call_deferred() 确保动画播放完成后执行转向操作，避免了动画播放与转向操作的竞态条件
...
- 注意事项
  ...
  - **新增**：PreEnding 触发器需要确保 GameManager 中存在相机跟随器
  - **新增**：动画器系统需要正确配置 AnimatorBase 的参数和信号连接
  - **更新**：ChangeSpeedTrigger 现在仅设置速度属性，不直接更新向量
...
```mermaid
flowchart TD
Enter["角色进入触发区域"] --> GetCamera["获取相机跟随器"]
GetCamera --> CheckCamera{"是否有相机跟随器?"}
...
- **更新**：触发器系统优化
  - **更新**：ChangeSpeedTrigger 移除了即时速度向量更新逻辑，现在仅设置基础速度属性
